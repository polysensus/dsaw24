...
contract BaseRouter is Router {
    ...
    // +-----------------------------------------------------------------------------------------+
    //
    function dispatch(bytes[] calldata actions, bytes calldata sig, uint256 nonce) public {
    ...
    function dispatchWithOpsRecorder(bytes[] calldata actions, bytes calldata sig, uint256 nonce)
        public
        returns (Op[] memory)
    {
        Session storage session;
        ...
        Context memory ctx = Context({sender: session.owner, scopes: session.scopes, clock: uint32(block.number)});
        // forward to the dispatcher registered with the session
        Op[] memory ops = session.dispatcher.dispatchWithOpsRecorder(actions, ctx);
        emit SeenOpSet(sig);
        return ops;
    }
