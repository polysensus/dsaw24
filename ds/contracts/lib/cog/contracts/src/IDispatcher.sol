...
import {Op} from "./StateOpsRecorder.sol";
...
interface Dispatcher {
    ...
    // Versions of dispatched used during simulation
    function dispatchWithOpsRecorder(bytes calldata action, Context calldata ctx) external returns (Op[] memory);
    function dispatchWithOpsRecorder(bytes[] calldata actions, Context calldata ctx) external returns (Op[] memory);

    // same as dispatch above, but ctx is built from msg.sender
    function dispatchWithOpsRecorder(bytes calldata action) external returns (Op[] memory);
    function dispatchWithOpsRecorder(bytes[] calldata actions) external returns (Op[] memory);
}
