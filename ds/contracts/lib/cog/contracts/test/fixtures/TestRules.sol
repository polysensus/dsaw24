...
import {State, CompoundKeyKind, WeightKind} from "../../src/IState.sol";
...
contract RegisterNodeAndEdgeType is Rule {
    function reduce(State s, bytes calldata action, Context calldata /*ctx*/ ) public returns (State) {
        if (bytes4(action) == TestActions.REGISTER_NODE.selector) {
            s.registerNodeType(bytes4(0xAABBCCDD), "NodeName", CompoundKeyKind.NONE);
        }
        if (bytes4(action) == TestActions.REGISTER_EDGE.selector) {
            s.registerEdgeType(bytes4(0xDDCCBBAA), "EdgeName", WeightKind.UINT64);
        }
        return s;
    }
}
