...
contract BaseRouterTest is Test {
    ...
    function setUp() public {
        state = new BaseState(address(this)); // TODO: replace with a mock
        dispatcher = new ExampleDispatcher(state, address(this));
        router = new BaseRouter();
        dispatcher.registerRouter(router);

        state.authorizeContract(address(dispatcher.registerRule(new LogSenderRule())));
    }
