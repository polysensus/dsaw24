...
contract ExampleDispatcher is Dispatcher, BaseDispatcher {
    constructor(State s, address _owner) BaseDispatcher(_owner) {
        _registerState(s);
    }
}
...
contract BaseDispatcherTest is Test {
    ...
    function setUp() public {
        s = new BaseState(address(this)); // TODO: replace with a mock
        d = new ExampleDispatcher(s, address(this));
    }

    function testReigsterRuleOrder() public {
        bytes memory action = abi.encodeCall(TestActions.NOOP, ());

        s.authorizeContract(address(d.registerRule(new LastWriteWinsRule("x"))));
        s.authorizeContract(address(d.registerRule(new LastWriteWinsRule("y"))));
        d.dispatch(action);
    ...
    function testActionArgs() public {
        ...
        s.authorizeContract(address(d.registerRule(new SetBytesRule())));
        d.dispatch(action);
    ...
    function testDispatchAsSender() public {
        ...
        s.authorizeContract(address(d.registerRule(new LogSenderRule())));
        vm.prank(sender);
        d.dispatch(action);
